...
static int client_process(struct client *client)
{
	...
	while ((n = recv(client->fd, buf, sizeof buf, 0)) > 0)
	{
		...
		{
		case SS_1_CONNECTED:
			am = check_auth_method(buf, n, client);
			if (am == AM_NO_AUTH)
				state = SS_3_AUTHED;
			else if (am == AM_USERNAME)
				state = SS_2_NEED_AUTH;
			send_auth_response(client->fd, 5, am);
			if (am == AM_INVALID)
				goto breakloop;
			break;
		case SS_2_NEED_AUTH:
			ret = check_credentials(buf, n);
			send_auth_response(client->fd, 1, ret);
			if (ret != EC_SUCCESS)
				goto breakloop;
			state = SS_3_AUTHED;
			if (auth_ips)
			...
			break;
		case SS_3_AUTHED:
			ret = connect_socks_target(buf, n, client);
			if (ret < 0)
			{
				send_error(client->fd, ret * -1);
				goto breakloop;
			}
			remotefd = ret;
			send_error(client->fd, EC_SUCCESS);
			copyloop(client->fd, remotefd);
			goto breakloop;
		}
	}
breakloop:
